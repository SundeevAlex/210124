...
def main():
    ...
    # is_aborted = False
    # while (basic_word.count_words() != len(player1.list_used_words)) and not is_aborted:

    # while (basic_word.count_words() != len(player1.list_used_words)) and flag:
    while basic_word.count_words() != player1.count_used_words():
        ...
        elif basic_word.check_word(user_word):
            if player1.check_word_before(user_word):
                print('Уже использовано!')
            else:
                print('Верно!')
                player1.add_used_word(user_word)
    ...
    #     print_rezults(player1.count_used_words() - 1)
    print_rezults(player1.count_used_words())
